...
! GItlink Raw Link: https://code.gitlink.org.cn/api/v1/repos/keytoolazy/adblock_auto/Rules/raw/${file##*/}
...
function modtify_adblock_original_file() {
	local file="${1}"
	local new=`cat "${file}" | grep -Ev '^#\@\?#|^\$\@\$|^#\%#|^#\@\%#|^#\@\$\?#|^#\$\?#|^<<|<<1023<<' | sed 's|^[[:space:]]@@|@@|g' | sort | uniq | sed '/^!/d;/^[[:space:]]*$/d;/^\[.*\]$/d' `
	echo "$new" > "${file}"
}
...
local file="${1}"
local IFS=$'\n'
local white_list_file="`pwd`/white_list/white_list.prop"
local list="$(cat ${white_list_file} | sort | uniq | sed '/^!/d;/^[[:space:]]*$/d')"
for i in ${list}
do
	sed -i "/${i}/d" "${file}"
done
...
| :-- | :-- | :-- |
| 混合规则(自动更新) | [订阅](https://raw.githubusercontent.com/lingeringsound/adblock_auto/main/Rules/adblock_auto.txt) | [订阅](https://code.gitlink.org.cn/api/v1/repos/keytoolazy/adblock_auto/Rules/raw/adblock_auto.txt) |
| 混合规则精简版(自动更新) | [订阅](https://raw.githubusercontent.com/lingeringsound/adblock_auto/main/Rules/adblock_auto_lite.txt) | [订阅](https://code.gitlink.org.cn/api/v1/repos/keytoolazy/adblock_auto/Rules/raw/adblock_auto_lite.txt) |
