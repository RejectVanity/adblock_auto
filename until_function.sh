...
local file="${1}"
local IFS=$'\n'
local list=`cat "`pwd`/white_list/white_list.prop" | sort | uniq | sed '/^!/d;/^[[:space:]]*$/d' `
for i in ${list}
do
	sed -i "/${i}/d" "${file}"
done
...
| :-- | :-- | :-- |
| 混合规则(自动更新) | [订阅](https://raw.githubusercontent.com/lingeringsound/adblock_auto/main/adblock_auto.txt) |
| 混合规则精简版(自动更新) | [订阅](https://raw.githubusercontent.com/lingeringsound/adblock_auto/main/adblock_auto_lite.txt) |
